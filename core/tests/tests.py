...
class SingleRace(TestCase):
    ...
    Manually insert a race into the DB, without using the uploader coder.
    ...
    def test_index_page(self):
        '''
        Sanity Check the index/root site returns the expected status code and check the context.
        '''
        client = Client()
        response = client.get('/')
    ...
    def test_single_race_details_page(self):
        '''
        Sanity Check the single_race_details page returns the expected status code and check the context.
        '''
        client = Client()
        response = client.get('/results/singleracedetail/{0}/'.format(self.singlerace.id))
        self.assertEqual(response.status_code, 200, 'Check the root page response code')
